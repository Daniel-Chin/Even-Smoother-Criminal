...
public record FirmSheet
{   // An amalgamation of the income statement and balance sheet.
    public record Event
    {
    ...
    public DateTime SheetPeriodStart { get; init; }
    public DateTime SheetPeriodEnd { get; init; }
    public string Currency { get; init; }
    public float BeginningAssets { get; init; }
    ...
    private static string Money(float v) => Money((decimal)v);

    public string Render()
    {
        var firm = E(FirmName ?? "");
        var periodStart = SheetPeriodStart.ToString("yyyy-MM-dd", CultureInfo.InvariantCulture);
        var periodEnd = SheetPeriodEnd.ToString("yyyy-MM-dd", CultureInfo.InvariantCulture);
...
");
        ...
        foreach (var ev in Events ?? new List<Event>())
        {
            var date = ev.DateStamp.ToString("yyyy-MM-dd", CultureInfo.InvariantCulture);
    ...
    public static FirmSheet Example()
    {
        ...
        {
            FirmName = "Example Consulting LLC",
            SheetPeriodStart = new DateTime(2024, 1, 1),
            SheetPeriodEnd = new DateTime(2024, 1, 31),
            Currency = "USD",
            BeginningAssets = 50000.00f,
            Events =
            [
                new() { DateStamp = new DateTime(2024, 1, 5), Description = "Sales to client Joshua Paiz", Amount = 150000.00f },
                new() { DateStamp = new DateTime(2024, 1, 10), Description = "Office rent to Sky City Holding", Amount = -20000.00f },
                new() { DateStamp = new DateTime(2024, 1, 15), Description = "Purchased raw materials from ABC Chemicals", Amount = -62000.00f },
                new() { DateStamp = new DateTime(2024, 1, 20), Description = "IPO", Amount = 300000.00f },
                new() { DateStamp = new DateTime(2024, 1, 25), Description = "Salary expense to John Doe", Amount = -1500.00f },
            ]
        };
    }
}
